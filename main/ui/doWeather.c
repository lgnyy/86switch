#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "nvs_cfg.h"
#include "cJSON.h"
#include "yos_http.h"
#include "zlib.h"
#include "doWeather.h"


static const char* weatcher_display_items[] = {"text", "temp", "windScale", "humidity", NULL};
static const char* weatcher_ca_cert = "";

// 解压函数
static int ungzip(char* src, int src_len, char* des, int des_len) {
    z_stream d_stream = { 0 };

    // 初始化解压流
    int ret = inflateInit2(&d_stream, 47);
    if (ret != Z_OK) {
        des[0] = '\0';
        return 0;
    }

    d_stream.next_in = (z_const Bytef*)src;
    d_stream.avail_in = src_len;
    d_stream.next_out = (Bytef*)des;
    d_stream.avail_out = des_len;

    // 解压数据
    int offset = 0;
    ret = inflate(&d_stream, Z_NO_FLUSH);
    if ((ret == Z_STREAM_END) && (d_stream.avail_out > 0)) {
        offset = des_len - d_stream.avail_out;
    }
    des[offset] = '\0';

    // 清理解压流
    (void)inflateEnd(&d_stream);
    return offset;
}

static void json_parse_display(const char* str, weather_display_cb_t display_cb, void* arg) {
    cJSON* root = cJSON_Parse(str);
    if (root == NULL) {
        return;
    }

    char* code = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(root, "code"));
    if ((code == NULL) || (strcmp(code, "200") != 0)) {
        goto end;
    }
   
    cJSON* now_json = cJSON_GetObjectItemCaseSensitive(root, "now");
    if (now_json == NULL) {
        goto end;
    }

    int i;
    for (i = 0; weatcher_display_items[i]; i++) {
        char* value = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(now_json, weatcher_display_items[i]));

        display_cb(arg, i, value);
    }
 
end:
    cJSON_free(root);
}


static int weather_load_config(void* ctx, nvs_cfg_read_cb_t read_cb, void* arg) {
    char city_pos[64], api_key[64];
    int ret = read_cb(arg, "city_pos", city_pos, sizeof(city_pos));
    if (ret == 0){
        ret = read_cb(arg, "api_key", api_key, sizeof(api_key));
        if (ret == 0){
            snprintf((char*)ctx, 256, "https://devapi.qweather.com/v7/weather/now?location=%s&key=%s&lang=cn", city_pos, api_key);
        }
    }
    return ret;
}

int weather_query(weather_display_cb_t display_cb, void* arg) {

    char url[256] = { 0 };
    int ret = nvs_cfg_load(NVS_CFG_WEATHER_INFO_NAMESPACE, weather_load_config, url);
    if (ret != 0) {
        strcpy(url, "https://devapi.qweather.com/v7/weather/now?location=116.39145,39.9073&key=c98d7cb82cdc49b7a20f8367e865868a&lang=cn");
        //return ret;
    }
 
    // 联机查询天气预报
    uint8_t* resp = NULL;
    uint32_t resp_len = 0;
    ret = yos_http_static_request(url, weatcher_ca_cert, NULL, NULL, 0, &resp, &resp_len);
    if (ret != 0) {
        return ret;
    }

    // 调用解压函数
    int des_len = resp_len * 4 + 256;
    char* des = (char* )malloc(des_len);
    if (des != NULL) {
        ungzip((char*)resp, (int)resp_len, des, des_len);

        printf("weather_query resp_len:%ld, des_len:%d, strlen:%d\n", resp_len, des_len, (int)strlen(des));
        
        json_parse_display(des, display_cb, arg);
    }
 
    yos_http_static_free(resp);
    free(des);
    return 0;
}
