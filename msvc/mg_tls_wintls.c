...
#ifndef MG_TLS_WINTLS
#define MG_TLS_WINTLS MG_TLS_CUSTOM
#endif // !MG_TLS_WINTLS
...
#define printe(...) MG_ERROR((__VA_ARGS__))
#define printd(...) MG_DEBUG((__VA_ARGS__))
#ifndef MIN
#define MIN(a, b) ((a) < (b) ? (a) : (b))
#endif

typedef struct mg_connection* WINTLS_SOCKET;
static int __sendwrapper(WINTLS_SOCKET fd, const char* buf, size_t len, int flags)
{
        int ret = mg_io_send(fd, buf, len);
        return ret;
}

static int __recvwrapper(WINTLS_SOCKET fd, char* buf, int len, int flags)
{
        int ret = mg_io_recv(fd, buf, len);
        if (ret == MG_IO_WAIT) {
...
hssl_ctx_t hssl_ctx_new(hssl_ctx_opt_t* opt)
{
        ...
        if (SecStatus == SEC_E_OK) {
                ...
                if (QueryCredentialsAttributesA(hCred, SECPKG_ATTR_SUPPORTED_ALGS, &algs) == SEC_E_OK) {
                        for (DWORD i = 0; i < algs.cSupportedAlgs; i++) {
...
hssl_t hssl_new(hssl_ctx_t ssl_ctx, WINTLS_SOCKET fd)
{
...
static int schannel_connect_step2(struct wintls_s* ssl)
{
        ...
        if (ssl->buffer_to_decrypt_offset_ > 0) {
                offset = ssl->buffer_to_decrypt_offset_;
                memcpy(buffer_in, ssl->buffer_to_decrypt_, offset);
...
#if 1 // ljc
                                        if (offset > 0) {
                                                memcpy(ssl->buffer_to_decrypt_, buffer_in, offset);
                                                ssl->buffer_to_decrypt_offset_ = offset;
                                        }
#endif
                                        ret = HSSL_WANT_READ;
                                }
...
int hssl_read(hssl_t _ssl, void* buf, int len)
{
        ...
        if (ssl->dec_len_ > 0) {
                ...
                int decrypted = MIN((int)(ssl->dec_len_), len);
        ...
        while (true) {
                ...
                if ((int)(ssl->buffer_to_decrypt_offset_) >= encrypted_buffer_len) {
...
void mg_tls_init(struct mg_connection* c, const struct mg_tls_opts* opts) {
        hssl_ctx_opt_t hssl_opts = { opts->cert.buf, opts->key.buf, opts->ca.buf };
        ...
        //hssl_opts.verify_peer = opts->skip_verification;
        hssl_opts.endpoint = c->is_client? HSSL_CLIENT: HSSL_SERVER;
   
        if ((tls->ctx = hssl_ctx_new(&hssl_opts)) == NULL) {
                mg_error(c, "hssl_ctx_new");
                goto fail;
        }
        if ((tls->ssl = hssl_new(tls->ctx, c)) == NULL) {
                mg_error(c, "hssl_new");
                goto fail;
        }
...
void mg_tls_handshake(struct mg_connection* c) {
        ...
        int rc = c->is_client ? hssl_connect(tls->ssl) : hssl_accept(tls->ssl);
        if (rc == 0) {
                ...
                int code = (rc == HSSL_WANT_READ)? 0 : rc;
...
size_t mg_tls_pending(struct mg_connection* c) {
        struct mg_tls* tls = (struct mg_tls*)c->tls;
        return tls == NULL ? 0 : (size_t)((struct wintls_s*)(tls->ssl))->dec_len_;
}

long mg_tls_recv(struct mg_connection* c, void* buf, size_t len) {
        struct mg_tls* tls = (struct mg_tls*)c->tls;
        int n = hssl_read(tls->ssl, buf, (int)len);
        if (n == 0) return MG_IO_WAIT;
...
long mg_tls_send(struct mg_connection* c, const void* buf, size_t len) {
        struct mg_tls* tls = (struct mg_tls*)c->tls;
        int n = hssl_write(tls->ssl, buf, (int)len);
        if (n == 0) return MG_IO_WAIT;
        if (n < 0) return MG_IO_ERR;
        return n;
}
